...
class ZhihuSpider(Spider):
    ...
    followers_query = 'data[*].answer_count,articles_count,gender,follower_count,is_followed,is_following,badge[?(type=best_answerer)].topics'  # 查詢參數
    ...
    def start_requests(self):
        ...
        yield Request(self.follows_url.format(user=self.start_user, include=self.follows_query, offset=0, limit=20 ), callback=self.parse_follows, headers=headers)  #關注人列表
        yield Request(self.followers_url.format(user=self.start_user, include=self.followers_query, offset=0, limit=20), callback=self.parse_followers, headers=headers) #粉絲列表
    ...
    #解析user的關注人頁面,取得關注人名稱並構造關注人url,翻頁及再遞迴取得資料
    ...
    #解析user的粉絲頁面,取得粉絲人名稱並構造粉絲url,翻頁及再遞迴取得資料
    def parse_followers(self,response):
        ...
        if 'paging' in results.keys() and results.get('paging').get('is_end') == False:
            next_page = results.get('paging').get('next')
            yield Request(next_page, self.parse_followers,headers=headers) #自己調用自己，形成翻頁
